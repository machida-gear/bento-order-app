...
/**
 * ユーザーごとの合計金額CSV出力API
 * GET /api/admin/reports/csv-by-user?start_date=XXX&end_date=XXX - ユーザーごとの合計金額CSVファイルをダウンロード
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const csvRows = userTotalsArray.map((userTotal) => {
      return [
        userTotal.employee_code,
        `"${userTotal.full_name}"`, // カンマを含む可能性があるためクォート
        userTotal.total_amount,
      ].join(',')
    })
    ...
    try {
      ...
      await supabaseAdmin.from('audit_logs').insert({
        ...
        details: {
          ...
          user_id: userId || null,
          user_count: userTotalsArray.length,
        },
    ...
    return new NextResponse(csvWithBOM, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="user_totals_${periodStartDate}_${periodEndDate}${vendorId ? `_vendor_${vendorId}` : ''}${userId ? `_user_${userId}` : ''}.csv"`,
      },
    })
  } catch (error) {
    console.error('CSV by user API error:', error)
