...
## 注文履歴ページの締日期間表示（2026-01-11追加）

### 判断

**注文履歴ページで、締日期間で区切って注文を集計・表示する。「先月」「今月」「来月」の切り替え機能を提供し、デフォルトは「今月」とする。**

### 理由

1. **会計・集計との一致**: 注文データはシステム設定の締日を基準に集計されるため、注文履歴もそれに合わせて表示する
2. **ユーザビリティ**: 月をまたぐ注文を確認しやすくなる
3. **過去と将来の確認**: 「先月」「来月」の切り替えにより、過去の注文と将来の予定注文を確認可能

### 実装

- サーバーコンポーネント（`app/(user)/orders/page.tsx`）で締日期間を計算
- クライアントコンポーネント（`components/orders-history-client.tsx`）で期間切り替えUIを表示
- URLパラメータ（`?period=previous|current|next`）で期間を指定可能
- システム設定の `closing_day` を基準に締日期間を自動計算

### 締日期間計算の注意点

- `calculateSingleClosingPeriod` 関数は「その月を**終了月**とする期間」を計算する
- 先月・来月の期間を計算する際は、**終了月**を基準に±1ヶ月する（開始月ではない）
- `order_date` がDateオブジェクトの場合もあるため、フィルタリング前にYYYY-MM-DD形式に変換する

---

## 開発環境のロックファイル問題解決（2026-01-11追加）

### 判断

**`predev`フックで`copy-fonts.js`を実行し、ロックファイルを削除することで開発サーバー起動時のロックエラーを解消する。**

### 理由

1. **バックグラウンド起動**: `npm run dev` をバックグラウンドで起動した場合、正常に終了せずロックファイルが残ることがある
2. **自動クリーンアップ**: 開発サーバー起動前に自動的にロックファイルを削除することで、手動操作を不要に
3. **シンプルな解決策**: 既存の`copy-fonts.js`にロックファイル削除機能を追加することで、追加のスクリプトを最小限に

### 実装

- `scripts/copy-fonts.js` にロックファイル削除機能を追加
- `package.json` の `predev` フックで `copy-fonts.js` を実行
- 削除に失敗した場合は警告を出力するが、処理を続行
