...
### サイトURL設定（メール内のリンクに使用）

- `NEXT_PUBLIC_SITE_URL`: 本番環境のサイトURL
  - **本番環境**: `https://bento-order-app-blond.vercel.app`
  - **開発環境**: 設定不要（デフォルトで `http://localhost:3000` が使用されます）
  - **用途**: 新規登録時の確認メールやパスワードリセットメール内のリンクURLに使用
  - **重要**: この環境変数を設定しないと、メール内のリンクがローカルURLになる場合があります
  - **注意**: Supabase Dashboardの`Site URL`設定も本番環境のURLに設定する必要があります（後述）

### データベース接続URL（オプション、パフォーマンス最適化用）

- `DATABASE_URL`: PostgreSQL接続文字列（Transaction connection使用）
  - **本番環境**: Supabase Dashboard > Project Settings > Database > Connection string > **Transaction** から取得
  - **形式**: `postgresql://postgres:[YOUR-PASSWORD]@db.[YOUR-PROJECT-REF].supabase.co:6543/postgres?pgbouncer=true`
  - **用途**: Transaction connection (6543)を使用した直接PostgreSQL接続（パフォーマンス向上）
  - **注意**: 
    - この環境変数はサーバーサイドでのみ使用可能です（クライアントサイドでは使用できません）
    - Transaction connection (6543)を使用することを強く推奨します（Direct connection (5432)より高速）
    - 既存のSupabaseクライアント（REST API経由）と併用可能
  - **IPv6接続問題**: 
    - Supabaseデータベースホスト（`db.xxxxx.supabase.co`）がIPv6アドレスのみを返す場合、Node.jsでDNS解決に失敗することがあります
    - **エラーメッセージ**: `getaddrinfo ENOTFOUND db.xxxxx.supabase.co`
    - **解決策**: Supabase Dashboard で「Session Pooler」の接続文字列を使用してください
      1. Supabase Dashboard > Project Settings > Database > Connection string
      2. 「Session」タブを選択
      3. 「Display connection pooler」をクリック
      4. 接続文字列をコピー（ホスト名が `aws-0-ap-northeast-1.pooler.supabase.com` のような形式になります）
  - **詳細**: [Transaction接続の使用方法.md](./Transaction接続の使用方法.md)を参照
...
> 補足: **Vercel Cron Jobs は GET で `/api/auto-order/run` を呼び出します**。そのため本番では GET でも実行できる実装になっています。  
> ただし手動実行は認証ヘッダーを付けやすい `POST` を推奨します。
...
5. **`NEXT_PUBLIC_SITE_URL`**
   - 使用箇所: `app/api/auth/signup/route.ts`, `app/(auth)/login/page.tsx`
   - 説明: 本番環境のサイトURL（メール内のリンクに使用）
   - 本番環境: `https://bento-order-app-blond.vercel.app`
   - 開発環境: 設定不要（デフォルトで `http://localhost:3000` が使用されます）

6. **`DATABASE_URL`**（オプション、パフォーマンス最適化用）
   - 使用箇所: `lib/database/pool.ts`, `lib/database/query.ts`
   - 説明: PostgreSQL接続文字列（Transaction connection使用）
   - 本番環境: Supabase Dashboard > Project Settings > Database > Connection string > **Transaction** から取得
   - 開発環境: 設定不要（既存のSupabaseクライアントを使用）
   - 詳細: [Transaction接続の使用方法.md](./Transaction接続の使用方法.md)を参照

### オプション環境変数（PDF生成用フォールバック）

6. **`PDF_SENDER_COMPANY`**
   - 使用箇所: `app/api/admin/orders/today/pdf/route.ts`
   - 説明: PDF送信者の会社名（`system_settings`テーブルに値がない場合のフォールバック）

7. **`PDF_SENDER_POSTAL_CODE`**
   - 使用箇所: `app/api/admin/orders/today/pdf/route.ts`
   - 説明: PDF送信者の郵便番号（フォールバック）

8. **`PDF_SENDER_ADDRESS`**
   - 使用箇所: `app/api/admin/orders/today/pdf/route.ts`
...
### メール確認URLがローカルアドレスになる場合

1. **`NEXT_PUBLIC_SITE_URL`環境変数の設定を確認**

   - Vercelの環境変数設定で`NEXT_PUBLIC_SITE_URL`が本番環境のURLに設定されているか確認
   - 例: `NEXT_PUBLIC_SITE_URL=https://bento-order-app-blond.vercel.app`
   - 設定後は必ず再デプロイしてください

2. **Supabase DashboardのSite URL設定を確認**

   - Supabase Dashboardにログイン
   - 左メニューから「Authentication」を選択
   - 「URL Configuration」セクションを開く
   - 「Site URL」を本番環境のURLに設定（例: `https://bento-order-app-blond.vercel.app`）
   - 「Redirect URLs」に本番環境のURLを追加（例: `https://bento-order-app-blond.vercel.app/**`）
   - 設定を保存
...
4. **メール確認を無効化する場合（開発環境のみ）**

   - Supabase Dashboard > Authentication > Settings
   - 「Enable email confirmations」のチェックを外す
   - **注意**: 本番環境ではメール確認を有効にすることを強く推奨します
...
### メール内のリンクがローカルURLになっている場合

1. **Supabase Dashboardの設定を確認**

   - Supabase Dashboardにログイン
   - プロジェクトを選択
   - 左メニューから「Authentication」→「URL Configuration」を開く
   - **Site URL**を本番環境のURLに設定（例: `https://bento-order-app-blond.vercel.app`）
   - 変更を保存
...
4. **Redirect URLsの設定**
   ...
   - **Redirect URLs**に本番環境のURLを追加（例: `https://bento-order-app-blond.vercel.app/**`）
   - 開発環境のURLも必要なら追加（例: `http://localhost:3000/**`）

### Supabaseメールテンプレートの設定

パスワードリセットメールなどのメールテンプレートでは、`{{ .ConfirmationURL }}`という変数が使用されます。この変数は**自動的に生成される**ため、手動で設定する必要はありません。

#### メールテンプレートの確認・編集方法

1. **Supabase Dashboardにログイン**
2. **左メニューから「Authentication」を選択**
3. **「Email Templates」を開く**
...
#### `{{ .ConfirmationURL }}`が正しく生成されるための設定

1. **Supabase Dashboardの設定**（上記の「メール内のリンクがローカルURLになっている場合」を参照）
   - Site URLを本番環境のURLに設定
   - Redirect URLsに本番環境のURLを追加

2. **環境変数の設定**
   - `NEXT_PUBLIC_SITE_URL`を本番環境のURLに設定

3. **コード側の設定**（既に実装済み）
   - `app/(auth)/login/page.tsx`の`handleResetPassword`関数で`redirectTo`パラメータを指定

#### 動作の仕組み

1. コード側で`redirectTo`パラメータを指定（例: `https://bento-order-app-blond.vercel.app/login?reset=true`）
2. Supabaseが`{{ .ConfirmationURL }}`を生成：
   - 実際のURLは `https://[PROJECT-ID].supabase.co/auth/v1/verify?token=...&type=recovery&redirect_to=[redirectUrl]` の形式
3. メール内のリンクに`{{ .ConfirmationURL }}`が含まれる
4. ユーザーがリンクをクリックすると、Supabaseの認証サーバーでトークンを検証
5. その後、`redirectTo`で指定されたURL（`/login?reset=true`）にリダイレクト

#### 注意事項

- `{{ .ConfirmationURL }}`は自動生成される変数で、手動で設定する必要はありません
- メールテンプレートは、Supabase Dashboard > Authentication > Email Templates で編集できます
- テンプレート内の`{{ .ConfirmationURL }}`は、そのまま使用してください（変更不要）

詳細は`docs/パスワードリセット機能の実装.md`を参照してください。
