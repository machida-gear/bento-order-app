# 自動注文機能の実装

このドキュメントでは、自動注文機能の実装詳細を記録します。

> 📖 **関連ドキュメント**: [README.md](./README.md) - すべてのドキュメントへの参照

---

## 実装概要

自動注文機能は、ユーザーが設定したテンプレートに基づいて、毎日自動的に翌営業日の注文を作成する機能です。

### 主な機能

1. **自動注文設定画面**: ユーザーがテンプレートを設定
2. **自動注文実行 API**: スケジューラーから呼び出されて注文を作成
3. **Vercel Cron Jobs**: 毎日自動的に実行

---

## 1. 自動注文設定画面

### 実装ファイル

- `app/(user)/settings/auto-order/page.tsx`: サーバーコンポーネント
- `app/(user)/settings/auto-order/auto-order-settings-client.tsx`: クライアントコンポーネント

### 機能詳細

#### 1.1 テンプレート一覧表示

- 曜日別（日曜日〜土曜日、毎日）にグループ化して表示
- 各テンプレートにメニュー名、数量、業者名を表示
- 編集・削除ボタン

#### 1.2 テンプレート追加・編集

- **曜日選択**: 日曜日〜土曜日、または毎日
- **メニュー選択**: 業者別にグループ化して表示
- **数量入力**: 1 以上の整数
- **保存・キャンセルボタン**

#### 1.3 テンプレート削除

- 確認ダイアログ付き削除機能

#### 1.4 重複チェック機能

**実装ロジック:**

```typescript
// 重複チェック関数
const checkDuplicate = (dayOfWeek: number | null): string | null => {
  // 編集時は現在編集中のテンプレートを除外
  const otherTemplates = templates.filter((t) => t.id !== editingId);

  // 毎日テンプレート（day_of_week = null）の場合
  if (dayOfWeek === null) {
    // 既に毎日テンプレートがあるかチェック
    const hasEveryday = otherTemplates.some((t) => t.day_of_week === null);
    if (hasEveryday) {
      return "毎日のテンプレートは既に設定されています。";
    }
    // 特定の曜日のテンプレートがあるかチェック
    const hasSpecificDay = otherTemplates.some((t) => t.day_of_week !== null);
    if (hasSpecificDay) {
      return "特定の曜日のテンプレートが既に設定されています。";
    }
  } else {
    // 特定の曜日のテンプレートの場合
    // 同じ曜日のテンプレートがあるかチェック
    const hasSameDay = otherTemplates.some((t) => t.day_of_week === dayOfWeek);
    if (hasSameDay) {
      return `${dayLabel}のテンプレートは既に設定されています。`;
    }
    // 毎日テンプレートがあるかチェック
    const hasEveryday = otherTemplates.some((t) => t.day_of_week === null);
    if (hasEveryday) {
      return "毎日のテンプレートが既に設定されています。";
    }
  }

  return null;
};
```

**チェック内容:**

1. **毎日テンプレートの重複**

   - 既に毎日テンプレートがある場合、新しい毎日テンプレートは追加不可

2. **特定の曜日の重複**

   - 既に特定の曜日（例：月曜日）のテンプレートがある場合、同じ曜日のテンプレートは追加不可

3. **毎日テンプレートと特定の曜日テンプレートの競合**
   - 毎日テンプレートがある場合、特定の曜日のテンプレートは追加不可
   - 特定の曜日のテンプレートがある場合、毎日テンプレートは追加不可

**UI での表示:**

- 既に設定されている曜日は選択不可（disabled）
- 選択不可の理由を表示
- 保存時に重複がある場合はエラーメッセージを表示

---

## 2. 自動注文実行 API

### 実装ファイル

- `app/api/auto-order/run/route.ts`

### 機能詳細

#### 2.1 認証

```typescript
// Vercel Cron Jobsからの呼び出しを確認
const isVercelCron = request.headers.get("x-vercel-cron") === "1";

// 開発環境や手動実行の場合は、Authorizationヘッダーで認証
if (!isVercelCron) {
  const authHeader = request.headers.get("authorization");
  const expectedSecret = process.env.AUTO_ORDER_SECRET;

  if (expectedSecret && authHeader !== `Bearer ${expectedSecret}`) {
    return NextResponse.json({ error: "認証が必要です" }, { status: 401 });
  }
}
```

- **Vercel Cron Jobs**: `x-vercel-cron`ヘッダーで自動認証
- **開発環境・手動実行**: `Authorization: Bearer <AUTO_ORDER_SECRET>`ヘッダーで認証

#### 2.2 翌営業日の判定

```typescript
// 最大30日先まで検索
for (let i = 1; i <= 30; i++) {
  const checkDate = new Date(today);
  checkDate.setDate(checkDate.getDate() + i);
  const checkDateStr = checkDate.toISOString().split("T")[0];

  const { data: orderDay } = await supabaseAdmin
    .from("order_calendar")
    .select("*")
    .eq("target_date", checkDateStr)
    .single();

  if (orderDay && orderDay.is_available) {
    targetDate = checkDateStr;
    break;
  }
}
```

- `order_calendar`テーブルから`is_available = true`の最初の日を取得
- 最大 30 日先まで検索

#### 2.3 既存注文チェック

```typescript
const { data: existingOrder } = await supabaseAdmin
  .from("orders")
  .select("id")
  .eq("user_id", user.id)
  .eq("order_date", targetDate)
  .eq("status", "ordered")
  .maybeSingle();

if (existingOrder) {
  // 既存注文がある場合はスキップ
  results.push({
    user_id: user.id,
    result: "skipped",
    detail: "既に注文があります",
  });
  continue;
}
```

- 対象日に既に注文（`status = 'ordered'`）がある場合はスキップ
- 手動注文を尊重する設計

#### 2.4 テンプレート適用

```typescript
// 対象日の曜日を取得（0=日曜、1=月曜、...、6=土曜）
const dayOfWeek = targetDateObj.getDay();

// テンプレートを取得（毎日テンプレートまたは該当曜日のテンプレート）
const { data: templates } = await supabaseAdmin
  .from("auto_order_templates")
  .select("*")
  .eq("user_id", user.id)
  .or(`day_of_week.is.null,day_of_week.eq.${dayOfWeek}`)
  .order("day_of_week", { ascending: true, nullsFirst: false });

// 優先順位: 特定の曜日のテンプレート > 毎日テンプレート
const specificDayTemplate = templates.find((t) => t.day_of_week === dayOfWeek);
const template =
  specificDayTemplate || templates.find((t) => t.day_of_week === null);
```

**優先順位:**

1. 特定の曜日のテンプレート（例：月曜日のテンプレート）
2. 毎日テンプレート（`day_of_week = null`）

#### 2.5 注文作成

```typescript
// 価格ID取得
const { data: priceId } = await supabaseAdmin.rpc("get_menu_price_id", {
  p_menu_id: template.menu_id,
  p_order_date: targetDate,
});

// 価格情報を取得（unit_price_snapshot用）
const { data: priceInfo } = await supabaseAdmin
  .from("menu_prices")
  .select("price")
  .eq("id", priceId)
  .single();

// 注文を作成
const { data: orderData } = await supabaseAdmin
  .from("orders")
  .insert({
    user_id: user.id,
    menu_item_id: template.menu_id,
    menu_price_id: priceId,
    order_date: targetDate,
    quantity: template.quantity,
    unit_price_snapshot: priceInfo.price,
    status: "ordered",
    source: "auto",
  })
  .select()
  .single();
```

- `get_menu_price_id`関数で価格 ID を取得
- 注文時の単価を`unit_price_snapshot`に保存
- `source = 'auto'`で自動注文であることを記録

#### 2.6 実行履歴記録

```typescript
// 実行履歴を作成
const { data: runRecord } = await supabaseAdmin
  .from("auto_order_runs")
  .insert({
    run_date: todayStr,
    executed_at: jstNow.toISOString(),
    status: "running",
    log_details: {
      target_date: targetDate,
      executed_at: jstNow.toISOString(),
    },
  })
  .select()
  .single();

// 実行履歴アイテムを記録
for (const result of results) {
  await supabaseAdmin.from("auto_order_run_items").insert({
    run_id: runRecord.id,
    user_id: result.user_id,
    target_date: targetDate,
    result: result.result, // 'created' | 'skipped' | 'error'
    detail: result.detail,
  });
}

// 実行履歴を更新
await supabaseAdmin
  .from("auto_order_runs")
  .update({
    status: "completed",
    log_details: {
      ...runRecord.log_details,
      created: createdCount,
      skipped: skippedCount,
      errors: errorCount,
      total: results.length,
    },
  })
  .eq("id", runRecord.id);
```

- `auto_order_runs`テーブルに実行履歴を記録
- `auto_order_run_items`テーブルにユーザーごとの実行結果を記録
- 結果: `created`（作成）、`skipped`（スキップ）、`error`（エラー）

---

## 3. Vercel Cron Jobs 設定

### 実装ファイル

- `vercel.json`

### 設定内容

```json
{
  "crons": [
    {
      "path": "/api/auto-order/run",
      "schedule": "0 10 * * *"
    }
  ]
}
```

- **パス**: `/api/auto-order/run`
- **スケジュール**: `0 10 * * *`（毎日 10:00 JST）
- Vercel にデプロイすると自動的に Cron Jobs が設定される

### タイムゾーン

- Cron Jobs のスケジュールは UTC 時間で指定
- `0 10 * * *`は UTC 10:00 = JST 19:00
- JST 10:00 に実行するには`0 1 * * *`（UTC 1:00）を指定する必要がある

**注意**: 現在の設定（`0 10 * * *`）は UTC 10:00 = JST 19:00 に実行されます。JST 10:00 に実行したい場合は、`vercel.json`を修正してください。

---

## 4. データベーステーブル

### auto_order_templates

| カラム名      | データ型          | 説明                                           |
| ------------- | ----------------- | ---------------------------------------------- |
| `id`          | `integer`         | 主キー                                         |
| `user_id`     | `uuid`            | ユーザー ID                                    |
| `menu_id`     | `integer`         | メニュー ID（`menu_items.id`を参照）           |
| `quantity`    | `integer`         | 数量                                           |
| `day_of_week` | `integer \| null` | 曜日（0=日曜、1=月曜、...、6=土曜、NULL=毎日） |
| `created_at`  | `timestamptz`     | 作成日時                                       |
| `updated_at`  | `timestamptz`     | 更新日時                                       |

### auto_order_runs

| カラム名      | データ型      | 説明                                   |
| ------------- | ------------- | -------------------------------------- |
| `id`          | `bigint`      | 主キー                                 |
| `run_date`    | `date`        | 実行日                                 |
| `executed_at` | `timestamptz` | 実行日時                               |
| `status`      | `text`        | ステータス（'running' \| 'completed'） |
| `log_details` | `jsonb`       | ログ詳細（JSON 形式）                  |

### auto_order_run_items

| カラム名      | データ型      | 説明                                          |
| ------------- | ------------- | --------------------------------------------- |
| `id`          | `integer`     | 主キー                                        |
| `run_id`      | `integer`     | 実行 ID（`auto_order_runs.id`を参照）         |
| `user_id`     | `uuid`        | ユーザー ID                                   |
| `target_date` | `date`        | 対象日                                        |
| `result`      | `varchar`     | 実行結果（'created' \| 'skipped' \| 'error'） |
| `detail`      | `text`        | 詳細情報                                      |
| `created_at`  | `timestamptz` | 作成日時                                      |

---

## 5. エラーハンドリング

### 各ユーザーごとのエラー処理

```typescript
for (const user of users) {
  try {
    // 自動注文処理
  } catch (error) {
    console.error(`Error processing user ${user.id}:`, error);
    results.push({
      user_id: user.id,
      result: "error",
      detail: error instanceof Error ? error.message : "不明なエラー",
    });
  }
}
```

- 各ユーザーの処理を try-catch で囲み、エラーが発生しても他のユーザーに影響しない
- エラーは実行履歴に記録される

### UNIQUE 制約違反の処理

```typescript
if (orderError) {
  // UNIQUE制約違反の場合は既に注文がある（レースコンディション）
  if (orderError.code === "23505") {
    results.push({
      user_id: user.id,
      result: "skipped",
      detail: "既に注文があります（レースコンディション）",
    });
  }
}
```

- レースコンディション（同時実行）による重複を防止
- UNIQUE 制約違反の場合はスキップとして記録

---

## 6. 動作確認

### 手動実行（開発環境）

```bash
curl -X POST http://localhost:3000/api/auto-order/run \
  -H "Authorization: Bearer <AUTO_ORDER_SECRET>"
```

### 実行履歴の確認

```sql
-- 実行履歴を確認
SELECT * FROM auto_order_runs ORDER BY executed_at DESC LIMIT 10;

-- ユーザーごとの実行結果を確認
SELECT
  aori.user_id,
  p.full_name,
  aori.target_date,
  aori.result,
  aori.detail
FROM auto_order_run_items aori
JOIN profiles p ON aori.user_id = p.id
WHERE aori.run_id = <run_id>
ORDER BY aori.user_id;
```

---

## 7. 注意事項

### 本番環境デプロイ時

1. **環境変数の設定**

   - `AUTO_ORDER_SECRET`を設定（`docs/環境変数設定手順.md`を参照）
   - Vercel ダッシュボードで環境変数を設定

2. **Cron Jobs の確認**

   - Vercel ダッシュボードで Cron Jobs が正しく設定されているか確認
   - スケジュールが正しいか確認（JST 10:00 に実行する場合は`0 1 * * *`）

3. **動作確認**
   - 自動注文実行後、`auto_order_runs`テーブルで実行履歴を確認
   - エラーが発生していないか確認

### タイムゾーンの注意

- Cron Jobs のスケジュールは UTC 時間で指定
- JST 10:00 に実行するには、UTC 1:00（`0 1 * * *`）を指定する必要がある
- 現在の設定（`0 10 * * *`）は UTC 10:00 = JST 19:00 に実行される

---

## 8. 参考ドキュメント

- [CHANGELOG.md](./CHANGELOG.md) - 変更履歴
- [PROGRESS.md](./PROGRESS.md) - 進捗状況
- [SPEC.md](./SPEC.md) - システム仕様書
- [本番環境デプロイ時のチェックリスト.md](./本番環境デプロイ時のチェックリスト.md) - デプロイ手順
- [環境変数設定手順.md](./環境変数設定手順.md) - 環境変数の設定方法

---

最終更新日: 2025-01-XX
